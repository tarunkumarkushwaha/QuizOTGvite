...
const ShowNewsEditModal = (props) => {
  ...
  const EditPost = async () => {
    ...
    try {
      ...
      formData.append("content", inputNews);

      // If file is a new upload, append it
      if (file.length && file[0].startsWith("blob:")) {
      ...
      refreshPosts(); // refresh all posts in parent
      onCancelClick();
    } catch (err) {
          ...
          >
            <div className="xl:mt-0 mt-32 relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-[600px] w-full">
              <div className="sm:items-start flex h-[30rem] flex-col justify-between p-4">
                {/* Textarea */}
                <div className="p-3 w-full flex gap-2.5">
                  <textarea
                    type="text"
                    value={inputNews}
                ...
                <div className="w-full flex gap-3 overflow-auto">
                  {file.map((item, index) => (
                    <div key={"image" + index} className="w-28 h-3/4 relative">
                      <button
                        className="absolute top-[-5px] right-[-5px] bg-red-500 rounded-full p-[2px]"
                        onClick={() => setFile([])}
                      >
                        <svg
                          ...
                          fill="white"
                        >
                          <path d="M5.1999 5.20002H14.7999V14.8H5.1999V5.20002Z" />
                          <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                      ...
                      <img src={item} alt="uploaded" className="rounded-md" />
                    </div>
                  ))}
                </div>
...
export default ShowNewsEditModal;