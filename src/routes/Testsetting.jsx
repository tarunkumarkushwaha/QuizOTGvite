...
import InterviewSetting from "./InterviewSetting.jsx";
...
const Testsetting = () => {
  ...
  const [interviewsetting, setinterviewsetting] = useState(false);
  const [questionGenerateInputText, setquestionGenerateInputText] =
    useState("");
  ...
  const generateQuestionsViaPdf = async () => {
    ...
    try {
      const response = await authFetch(`/ask/generate-from-pdf`, {
        method: "POST",
        body: formData,
        headers: {
  ...
  const startTest = () => {
    if (
      ...
      (!Array.isArray(TestQuestion) || TestQuestion.length === 0)
    ) {
  ...
  const submitMultipleQuestions = async () => {
    ...
    const subject = questionGenerateInputText
      ? questionGenerateInputText.slice(0, 20)
      : file?.name?.split(".")[0];
    // if (!subject) {
    ...
    <>
      ...
      focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <div className="mt-8 p-6 rounded-xl bg-white/5 border border-white/10">
                    ...
                    {questionGenerateInput && (
                      <TextField
                        ...
                        onChange={(e) =>
                          setquestionGenerateInputText(e.target.value)
                        }
                        fullWidth
                ...
                <div className="mt-10 flex justify-center gap-4">
                  <button
                    type="button"
                    onClick={() => setinterviewsetting(true)}
                    disabled={loading}
            ...
            <InterviewSetting setinterviewsetting={setinterviewsetting} />
          )
        ) : (
          <LoadingSpinner />
        )}
