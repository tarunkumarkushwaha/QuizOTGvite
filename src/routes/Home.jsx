...
const Home = () => {
  const { signIn, setdbCustomQuestions, getdbQuestions } = useContext(Context);
    ...
    <>
      ...
      <div className="mainbg bg-no-repeat bg-left h-screen">
        <div className="bg-slate-950/60 h-screen flex justify-center m-0 item-center flex-col">
          ...
          {/* test  */}
          {/* <button onClick={setdbCustomQuestions}
            className="h-10 sm:w-48 w-44 text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 font-medium rounded-lg sm:text-sm text-xs px-5 py-2.5 text-center"
          >setdata</button>

          <button onClick={getdbQuestions}
            className="h-10 sm:w-48 w-44 text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 font-medium rounded-lg sm:text-sm text-xs px-5 py-2.5 text-center"
          >getdata</button> */}
